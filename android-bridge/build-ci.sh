...
"$bridge_tools/zipalign" -f -p 4 "$bridge_work/unsigned.apk" "$bridge_out/bunjang-bridge-0.3-unsigned.apk"
...
"$bridge_tools/aapt" dump badging "$bridge_out/bunjang-bridge-0.3-unsigned.apk" > "$bridge_out/package-info.txt"
(cd "$bridge_out" && sha256sum bunjang-bridge-0.3-unsigned.apk apksigner-android.zip > SHA256SUMS)
